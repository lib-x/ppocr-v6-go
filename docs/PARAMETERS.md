...
- **动态维度**表示这些轴在运行时确定。本库用的 `pure-onnx` binding 要求调用方提供**精确匹配**的输出张量形状（ONNX Runtime 的 C API 允许传 nullptr 由运行时分配，Python 的 `session.run(None, ...)` 即是），所以库用"形状探测"这个 workaround 获取真实尺寸（见第 4 节）。
...
## 2. 预处理参数（模型文件里没有，来自部署契约）

ONNX 只描述网络计算，不描述图像怎么变成输入张量。这部分参数的第一手来源是**模型仓库自带的 `inference.yml`**（PP-OCRv6 的两个模型仓库里都有，和 `inference.onnx` 并列），它是随模型分发的部署契约；其次才是 PaddleOCR/PaddleX 的源码和配置。注意生效顺序：PaddleX 的 predictor 先用一个默认 reader，随后会被配置里的 `DecodeImage` 覆盖，所以看构造函数里的默认值是会看错的，要看配置。
...
| 通道顺序 | BGR（默认，`Config.RGB` 可切换） | BGR（固定） | 模型自带的 `inference.yml`：`DecodeImage.img_mode: BGR`。注意 PaddleX predictor 构造函数里的 `ReadImage(format="RGB")` 只是兜底，`_build` 会用配置里的 `DecodeImage` 覆盖它 |
...
- **识别模型**：输入宽度 W 决定 CTC 序列长度 T（与骨干网络的下采样有关，实测 43 个宽度全部满足 `T = floor((W+3)/8)`，常见宽度下就是 `W/8` 上下差 1；宽度 714 对应 89 帧、320 对应 40 帧）。库先用一个故意超大的输出张量运行一次，让 ORT 报错并给出真实形状，解析后按宽度缓存（`Recognizer.outputLen`）。
...
|---|---|---|---|
| 二值化阈值 `Threshold` | 0.2 | 模型契约 0.2；老命令行 0.3；PaddleX 兜底 0.3 | 概率图是逐像素的"文字概率"，取低值偏向召回：先多留候选，误检交给框级过滤兜底 |
| 框级阈值 `BoxThresh` | 0.45 | 模型契约 0.45；老命令行 0.6；PaddleX 兜底 0.6；`DBPostProcess` 类默认 0.7 | 计算框内概率均值，低于阈值整框丢弃，用来滤掉"零星亮、整体不像文字"的误检 |
| 最小框面积 `MinBoxArea` | 10 px | 参考用短边 `min_size=3` | 去掉二值化后的噪点小块，二者效果接近 |
| 最小边长 `MinBoxSide` | 3 px | 3（unclip 后另判 5） | 同上，按短边过滤细碎框 |
| unclip 比例 `UnclipRatio` | 1.4 | 模型契约 1.4；老命令行 1.5；PaddleX 兜底 2.0 | 训练时文字区域按 `D = A(1-r²)/L`（r=0.4）收缩后再让模型学，推理端按另一个公式 `D = A*unclip_ratio/L` 放大补偿，两者不是互逆关系。本库跟模型契约取 1.4；实测比例偏大的代价明显（同一张菜单 1.5 时均分 0.9489、2.0 时 0.9347） |
...
| 参数 | 为什么是这个值 | 改大/改小的后果 |
|---|---|---|
| 检测 `MaxSideLen` 960 | 官方对 PP-OCRv6 的部署配置是长边 960（`resize_long=960`），速度与精度的折中 | 耗时实测：480 / 960 / 1600 对同一张 970x1417 菜单图是 279ms / 638ms / 1796ms。召回只有弱证据：菜单缩半后 1600 比 960 多 1 个框（35 对 34），原图三者都是 34 个框 |
| 补白到 32 的倍数 | 检测网络的总下采样步长是 32（每 32 像素汇总成一个特征点）。参考实现是把尺寸 round 到 32 的倍数，本库改成向右下补白，避免为对齐而轻微拉伸图像 | 不补齐不是"精度略降"而是直接报错：实测把 500x300（非 32 倍数）喂给检测模型，ORT 1.24.1 在 Conv 节点上以特征图尺寸不匹配失败；512x320、672x960 正常 |
| 检测归一化 ImageNet 均值方差 | 模型契约（`inference.yml` 的 `NormalizeImage`）写的就是 ImageNet 统计量，输入分布必须一致 | 换成识别用的 0.5/0.5：概率图整体变弱、条带断裂 |
| 识别高度 48 | 训练配置 `d2s_train_image_shape: [3,48,320]`，模型按高 48 学到的字形模式 | 改高度：轻则精度下降，重则构造期校验直接报错 |
| 识别宽度 `ceil(48*w/h)` | 等比缩放保持字形比例；`MaxWidth` 可改为固定宽度 | 实测"压缩必然掉分"不成立：669 宽的行压到 320 丢一个空格（0.9822→0.9713），压到 480 反而升（→0.9870）；菜单两行压到 320 一升一平。训练宽度在 320 一带（MultiScaleSampler scales 320x32/48/64），长行等比缩放后可能离训练分布更远 |
| 识别归一化 `(x/255-0.5)/0.5` | 把 0..255 映射到 -1..1，训练时的数值范围 | 用检测的 ImageNet 参数：识别结果基本崩掉 |
| CTC blank = 0、空格 = 18709 | 字典 18708 行，加 blank 和空格共 18710 类，和模型输出最后一维一致；模型自带的 `inference.yml` 里也内嵌了同一份字典，可逐条比对（已比对一致） | 映射错位：整段文字变成乱码，所以构造时会校验类别数 |
| 置信度取均值 | 和 PaddleOCR `CTCLabelDecode` 一致（`np.mean(conf_list)`） | 换成概率乘积：长文本的分数会被指数级压低，阈值失去可比性 |
| 形状探测缓存 | `pure-onnx` 的 `RunWithValues` 要求预分配精确形状的输出张量（ORT 的 C API 本身允许传 nullptr 由运行时分配，这是 binding 的限制），探测要跑一次"故意失败"的推理 | 不缓存：每次调用多一次毫秒级失败推理，批量处理时浪费明显 |
